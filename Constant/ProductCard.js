        ...
        <View className="mt-4">
            <FlatList horizontal showsHorizontalScrollIndicator={false}
                ...
                renderItem={({ item, index }) => (
                    <TouchableOpacity onPress={() => nav.navigate("ProductDetail", { item })} activeOpacity={0.7} key={index} className="mr-3 border-2 border-[#E3E3E3] rounded-2xl h-[240px] w-[180px]">
                        ...
                        <View className="flex flex-col justify-between space-y-1 h-[50%] p-2">
                            ...
                            <Text className="text-base text-black font-PoppinsSemiBold">{item.pieces} Kg</Text>
                            <View className="flex flex-row items-center justify-between">
                                <Text className="text-base text-black font-PoppinsMedium">₹ {item.price}</Text>
